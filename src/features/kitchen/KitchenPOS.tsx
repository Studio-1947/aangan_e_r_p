              ...
              <TabsContent key={category} value={category} className="mt-0">
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                  {items.map((item) => (
                    <Card
                      key={item.id}
                      className="group cursor-pointer border-slate-200 transition hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md"
                      onClick={() => addMenuItem(item)}
                    >
                      <CardContent className="flex items-center justify-between gap-3 p-4">
                        <div>
                          <h3 className="font-semibold text-slate-950">{item.name}</h3>
                          <p className="text-sm text-slate-500">Tap to add to cart</p>
                        </div>
      ...
      <Card className="sticky top-6 h-fit overflow-hidden border-slate-200 shadow-lg shadow-slate-200/60">
        ...
        <CardContent className="space-y-5 p-5">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-700">Active Guest</label>
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
            {selectedGuestDetails ? (
              <>
                Billing to <span className="font-semibold text-slate-950">{selectedGuestDetails.name}</span> in room {selectedGuestDetails.roomLabel}.
              </>
            ) : (
          ...
          <ScrollArea className="max-h-[420px] rounded-2xl border border-slate-200 bg-white">
                    ...
                    >
                      ...
                      <div className="mt-4 flex items-center justify-between gap-3">
                        <div className="inline-flex items-center rounded-full border border-slate-200 bg-slate-50 p-1">
                          ...
                          <span className="min-w-10 px-3 text-center text-sm font-semibold text-slate-950">
                            {item.quantity}
                          </span>
                          <Button
                            size="icon"
